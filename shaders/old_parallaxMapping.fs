...
void main(){
    float numLayer=mix(10,8,abs(ViewDir.z));
    
    float size=1./numLayer;
    
    float layer=0.;
    
    vec2 P=ViewDir.xy/ViewDir.z*.01;
    vec2 d=P/numLayer;
    vec2 uv=TexCoord;
    ...
    vec2 finalUV=uv2*weight+uv*(1.-weight);
    if(finalUV.x>1.||finalUV.y>1.||finalUV.x<0.||finalUV.y<0.){
