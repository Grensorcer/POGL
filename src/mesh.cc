...
namespace mygl
{
                                     ...
                                     const std::vector<unsigned int> &indices)
    {
        ...
        for (size_t i = 0; i < neighbour_indices_.size(); ++i)
        {
            ...
            std::cout << '[' << i << ']' << " at " << vertices[i] << ":\n"
                      << "\t" << neighbour_indices[i * 4] << " at "
                      << vertices[neighbour_indices[i * 4]] << '\n'
                      << "\t" << neighbour_indices[i * 4 + 1] << " at "
                      << vertices[neighbour_indices[i * 4 + 1]] << '\n'
                      << "\t" << neighbour_indices[i * 4 + 2] << " at "
                      << vertices[neighbour_indices[i * 4 + 2]] << '\n'
                      << "\t" << neighbour_indices[i * 4 + 3] << " at "
                      << vertices[neighbour_indices[i * 4 + 3]] << '\n';
        }
